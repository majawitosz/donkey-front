...
const CATEGORY_CONFIG = {
	schedule: {
		...
		emptyHint:
			'Wygeneruj grafik lub wybierz inną datę, aby zobaczyć zaplanowane zmiany.',
	...
	vacation: {
		...
		emptyHint:
			'Gdy tylko urlop zostanie zaakceptowany pojawi się w tym kalendarzu.',
	...
	training: {
		label: 'Szkolenia',
		description:
			'Szkolenia i wydarzenia rozwojowe zaplanowane dla zespołu.',
		emptyTitle: 'Brak szkoleń w wybranym dniu',
		emptyHint:
			'Dodaj szkolenie lub wybierz inny termin, aby zobaczyć plan działań rozwojowych.',
	...
	medical: {
		...
		emptyHint:
			'Zaplanowane badania pracowników będą widoczne w tej zakładce.',
...
export function CalendarDashboard({
	...
	const [activeTab, setActiveTab] =
		React.useState<CalendarCategory>('schedule');
	...
	return (
		<div className='space-y-8'>
			<div className='space-y-4'>
				<div className='flex flex-col gap-2'>
					<h1 className='text-2xl font-semibold tracking-tight'>
						Kalendarze i widoki czasowe
					</h1>
					<p className='text-sm text-muted-foreground max-w-2xl'>
						Centralne miejsce do przeglądu grafików, urlopów,
						szkoleń oraz obowiązkowych badań pracowników. Użyj
						zakładek, aby szybko przełączać się między
				...
				{error ? (
					<Alert variant='destructive'>
						...
						<AlertDescription>
							{error}. Upewnij się, że masz aktywną sesję
							(wymagana autoryzacja) i spróbuj odświeżyć stronę.
						</AlertDescription>
					</Alert>
				) : null}
			</div>
			...
			<Tabs
				value={activeTab}
				onValueChange={(value) =>
					setActiveTab(value as CalendarCategory)
				}>
							...
							<div className='mt-2 flex flex-wrap items-center gap-2 text-xs text-muted-foreground'>
								...
								{event.status ? (
									<Badge variant='secondary'>
										{formatMedicalStatus(event.status)}
									</Badge>
								) : null}
							</div>
						) : null}
	...
	if (integrations.length === 0) {
		return (
			<Card className='border-dashed'>
				<CardHeader>
					<CardTitle>
						Synchronizacja z kalendarzami zewnętrznymi
					</CardTitle>
					<CardDescription>
						Podłącz kalendarz Google lub Outlook, aby automatycznie
						udostępniać grafiki i urlopy całemu zespołowi.
					</CardDescription>
				</CardHeader>
				<CardContent>
	...
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription>
					Zarządzaj połączeniami i decyduj, które kalendarze mają
...
function detectCategory(event: CalendarEvent): CalendarCategory {
	...
	if (
		typeValue.includes('vac') ||
		typeValue.includes('leave') ||
		titleValue.includes('urlop')
	) {
		return 'vacation';
	}
	if (
		typeValue.includes('train') ||
		titleValue.includes('szkole') ||
		titleValue.includes('trening')
	) {
		return 'training';
	}
	if (
...
function isAllDayEvent(start: Date | null, end: Date | null): boolean {
	...
	return (
		isSameDay(start, end) && start.getHours() === 0 && end.getHours() === 0
	);
}
...
function collectEventDays(events: EnrichedCalendarEvent[]): Date[] {
	...
	events.forEach((event) => {
		...
		const safeEnd = isAfter(intervalEnd, intervalStart)
			? intervalEnd
			: intervalStart;
...
function formatEventTime(event: EnrichedCalendarEvent): string {
	...
	if (!event.endDate || isSameDay(event.startDate, event.endDate)) {
		return `${format(event.startDate, 'HH:mm', { locale: pl })}${
			event.endDate
				? ` – ${format(event.endDate, 'HH:mm', { locale: pl })}`
				: ''
		}`;
	}
	return `${format(event.startDate, 'd MMM HH:mm', {
		locale: pl,
	})} – ${format(event.endDate, 'd MMM HH:mm', { locale: pl })}`;
}
